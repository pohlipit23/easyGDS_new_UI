...
const server = http.createServer((req, res) => {
    ...
    if (pathname.startsWith('/api/')) {
        ...
        delete proxyOptions.headers['host']; // https module handles this usually, but good to be explicit or let it auto-set
        // Actually, for https request, we shouldn't set Host manually in headers if we want SNI to work properly sometimes,
        // but often we need to override it if we are transparent proxying. 
        // Let's set the proper Host header.
...
server.listen(PORT, () => {
    ...
    console.log(`>>> Open http://localhost:${PORT}/all_forms_demo.html to test.\n`);
});
